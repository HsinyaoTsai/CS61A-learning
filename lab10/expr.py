...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError('name {0} is not defined'.format(self.string))
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        opas = [o.eval(env) for o in self.operands]
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        inner_env = self.parent.copy()
        for p, a in zip(self.parameters, arguments):
            inner_env[p] = a
        return self.body.eval(inner_env)
